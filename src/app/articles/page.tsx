...
export default function ArticlesPage() {
  ...
  const { resources, meta, loading, error, refetch } = useResources({
    contentType: 'article', skillLevel, sortBy,
  });
    ...
    <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '100px 24px 80px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ fontSize: '1rem', color: 'var(--text-secondary)', fontWeight: 300 }}>
          {loading ? 'Fetching articles...' : `${meta.total} articles from Art of Lock Picking, r/lockpicking, Medium, and more`}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', margin: '20px 0 32px', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {(['beginner','intermediate','advanced'] as SkillLevel[]).map(level => (
            <button key={level} onClick={() => setSkillLevel(skillLevel === level ? undefined : level)} style={{ padding: '7px 14px', borderRadius: 'var(--radius)', border: `1px solid ${skillLevel === level ? 'var(--blue)' : 'var(--border)'}`, background: skillLevel === level ? 'var(--blue-dim)' : 'var(--surface)', color: skillLevel === level ? 'var(--blue)' : 'var(--text-secondary)', fontSize: '0.82rem', cursor: 'pointer', transition: 'all 0.15s ease', textTransform: 'capitalize' as const }}>
              {level}
            </button>
          ))}
        </div>
